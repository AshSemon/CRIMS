...
if(isset($_GET['type']) && $_GET['type']!=='' && isset($_GET['id']) && $_GET['id']>0){
    ...
    if($type=='delete'){
		mysqli_query($con,"delete from returned where id='$id'");
		redirect('case.php');
	}
	if($type=='active' || $type=='deactive'){
		...
		mysqli_query($con,"update returned set status='$status' where id='$id'");
		redirect('case.php');
	}

}

$sql="select * from returned where detective_id='".$_SESSION['DETECTIVE_ID']."' order by id desc";
        ...
        <div class = "item-list">
                ...
                <div class = "item">
                    ...
                    <div class = "item-detail">
                        <span href = "#" class = "item-name">Case No:  #<?php echo $row['caseid']?></span>
                        ...
                        <span href = "#" class = "item-name">Prosecutor :  <?php echo $row['prosecutor']?></span>
                        ...
                        <?php
                            echo "<h5>Archive :- ".getArchiveById($row['achive_id'])."</h5>"
                        ?>
                        <?php if($row['investigation']==''){ ?>
                            <a href="manage_return.php?id=<?php echo $row['id']?>"><label class="btn btn-success mr-2 hand_cursor">Add Investigation</label></a>
                        <?php }else{ ?>
                            <?php if($row['achive_id']=='0'){ ?>
                                <a href="manage_return.php?id=<?php echo $row['id']?>"><label class="btn btn-secondary mr-2 hand_cursor">Edit Investigation</label></a>&nbsp;
                            <?php }else{ ?>
